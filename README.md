...
一个能接收用户复杂需求、自动拆解任务、调度最优大模型、执行并验证结果的 简单的Agent 编排框架。底层对接阿里云百炼模型，提供 Streamlit 前端和 FastAPI 异步任务两种运行模式。

此项目用意为消化我的上一段实习经历，主要的逻辑结构为：输入命令，Agent识别意图，挑选适配大模型，返回模型生成结果。

主要的调度Agent不参与任何任务，只负责数据调度，和跟新状态机，单Agent调度多模型。
